...
def main():
    ...
    Y_train_bronze = Y_train[:, 0]
    Y_train_silver = Y_train[:, 1]
    Y_train_gold = Y_train[:, 2]

    Y_test_bronze = Y_test[:, 0]
    Y_test_silver = Y_test[:, 1]
    Y_test_gold = Y_test[:, 2]
    ...
    for medal, y in {
        "bronze": Y_train_bronze,
        "silver": Y_train_silver,
        "gold": Y_train_gold
    }.items():
        pipeline = Pipeline([
        ...
        models[medal] = pipeline

    # evaluating model accuracy

    Y_pred = np.column_stack([
        models["bronze"].predict(X_test),
        models["silver"].predict(X_test),
        models["gold"].predict(X_test)
    ])
    ...
    Y_2028_pred = np.column_stack([
        models["bronze"].predict(X_2028),
        models["silver"].predict(X_2028),
        models["gold"].predict(X_2028)
    ])
    ...
    pred_2028['Pred_TM'] = pred_2028['Pred_TM'].apply(
        lambda x: np.clip(np.round(x), 0, None).astype(int)
    )
